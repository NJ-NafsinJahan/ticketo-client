      ...
      <nav className="flex-grow overflow-y-auto px-3 py-4 space-y-1">
        ...
        {menuItems.map(({ key, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <span
                className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors ${isActive ? "bg-gradient-to-br from-pink-500 to-indigo-600 text-white shadow-md shadow-pink-500/20" : "bg-white/5 text-slate-400"}`}
              >
                <Icon size={14} />
              </span>
              <span>{label}</span>
    ...
    <>
      ...
      <div className="lg:hidden w-full flex items-center justify-between px-6 py-4 bg-slate-950/80 border-b border-white/5 backdrop-blur-xl sticky top-0 z-50">
        ...
        <Drawer>
          <Button
            variant="bordered"
            className="border-white/10 hover:border-white/20 text-white font-semibold flex items-center gap-2 min-w-0 px-3 py-1.5 h-10"
            radius="lg"
          >
            <FaBars />
            Menu
          ...
          <Drawer.Backdrop>
            <Drawer.Content
              placement="left"
              className="bg-slate-950/95 border-r border-white/5 backdrop-blur-xl p-0 w-64 max-w-xs"
            >
